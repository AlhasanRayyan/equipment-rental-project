                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-muted small">
                                                    {{ Str::limit($item->category->category_name ?? 'غير محدد', 20) }}
                                                </div>
                                            </div>
                                    </td>
                                    <td>{{ $item->owner->first_name ?? 'غير معروف' }} {{ $item->owner->last_name ?? '' }}
                                    </td>
                                    <td>
                                        ...
                                        <div class="dropdown action-dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                        data-bs-target="#viewEquipmentModal{{ $item->id }}"><i
                                                            class="fas fa-eye text-primary"></i> عرض التفاصيل</a></li>
                                                @if (!$item->is_approved_by_admin)
                                                    <li><a class="dropdown-item" href="#"
                                                            onclick="event.preventDefault(); document.getElementById('approve-form-{{ $item->id }}').submit();"><i
                                                                class="fas fa-check-circle text-success"></i> الموافقة على
                                                            المعدة</a></li>
                                                    <form id="approve-form-{{ $item->id }}"
                                                        action="{{ route('admin.equipment.approve', $item) }}"
                                                        method="POST" class="d-none">@csrf</form>
                                                    <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                            data-bs-target="#rejectEquipmentModal{{ $item->id }}"><i
                                                                class="fas fa-times-circle text-danger"></i> رفض المعدة</a>
                                                    </li>
                                                @endif
                                                <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                        data-bs-target="#deleteEquipmentModal{{ $item->id }}"><i
                                                            class="fas fa-trash text-danger"></i> حذف المعدة</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                @if ($item->images->count() > 0)
                                    <img src="{{ $item->images->first()->image_url }}" alt="صورة المعدة"
                                        class="img-fluid rounded shadow-sm">
                                @else
                                    <img src="{{ asset('assets/img/default-equipment.png') }}" alt="صورة افتراضية"
                                        class="img-fluid rounded shadow-sm">
                                @endif
                            </div>
                            <div class="col-md-6">
                                ...
                                <p><strong>المالك:</strong> {{ $item->owner->first_name ?? 'غير معروف' }}
                                    {{ $item->owner->last_name ?? '' }}</p>
                                ...
                                @if ($item->last_maintenance_date)
                                    <p><strong>تاريخ آخر صيانة:</strong>
                                        {{ $item->last_maintenance_date->format('Y-m-d') }}</p>
                                    <p><strong>ملاحظات الصيانة:</strong> {{ $item->maintenance_notes }}</p>
                                @endif
                                <p><strong>تتبع GPS:</strong>
                                    @if ($item->has_gps_tracker)
