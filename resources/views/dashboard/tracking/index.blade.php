                    ...
                    <div class="card-body p-3">
                        @if ($trackings->isEmpty())
                            <div class="alert alert-info mb-0">
                                لا توجد بيانات تتبع حالياً داخل نطاق غزة.
                            </div>
                        @else
...
@section('content')
    @php
        ...
        $focusedEquipmentId = request('equipment_id');
    @endphp
    ...
    @if ($trackings->isNotEmpty())
        <style>
            ...
            .filter-chip input[value="idle"]+span i {
                color: #ffc107;
            }
        ...
        <script>
            window.trackingData = @json($trackingData);
            window.focusedEquipmentId = @json($focusedEquipmentId);
        </script>
        ...
        <script>
            async function initMap() {
                ...
                function buildContent(item, config) {
                    return `
                        <div class="card border-0 shadow rounded-4 overflow-hidden" style="min-width: 280px;">
                            <div class="card-header bg-light d-flex justify-content-between align-items-center py-3 px-3">
                                <div>
                ...
                window.trackingData.forEach(item => {
                    ...
                    const content = buildContent(item, config);
                ...
                if (window.focusedEquipmentId) {
                    const target = markers.find(entry => String(entry.item.equipment_id) === String(window
                        .focusedEquipmentId));

                    if (target) {
                        map.setCenter({
                            lat: Number(target.item.latitude),
                            lng: Number(target.item.longitude)
                        });
